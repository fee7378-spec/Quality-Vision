...
export const AnalystModal: React.FC<AnalystModalProps> = ({ analystCode, analystName, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-gray-900/80 dark:bg-black/85 backdrop-blur-sm animate-fadeIn">
      <div className="bg-white dark:bg-[#131b2e] w-full max-w-4xl rounded-2xl shadow-2xl border border-gray-200 dark:border-gray-800 overflow-hidden flex flex-col max-h-[90vh]">
        {/* Header */}
        <div className="p-5 bg-white dark:bg-[#131b2e] border-b border-gray-200 dark:border-gray-800 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="p-3 bg-brand-blue/10 border border-brand-blue/30 text-brand-blue-light dark:text-blue-400 rounded-xl">
              <User size={22} />
            </div>
            <div>
              <h2 className="text-lg font-black text-gray-900 dark:text-white tracking-wide uppercase">{nameDisplay}</h2>
              <div className="flex items-center gap-2 text-xs text-gray-500 dark:text-gray-400 mt-0.5">
                <span className="font-mono bg-gray-100 dark:bg-[#192238] px-2 py-0.5 rounded text-brand-blue-light dark:text-blue-300">{codeDisplay}</span>
                <span>•</span>
          ...
          <button
            onClick={onClose}
            className="p-2 text-gray-500 hover:text-gray-900 dark:text-gray-400 dark:hover:text-white bg-gray-100 dark:bg-[#192238] hover:bg-gray-200 dark:hover:bg-[#243049] rounded-lg transition-colors cursor-pointer"
          >
            <X size={18} />
          </button>
        </div>
        ...
        <div className="p-6 overflow-y-auto space-y-6 custom-scrollbar">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <div className="bg-white/90 dark:bg-[#192238] border border-gray-200 dark:border-gray-800 p-4 rounded-xl flex items-center justify-between">
              <div>
                <p className="text-xs text-gray-500 dark:text-gray-400 font-semibold uppercase">Total Monitorias</p>
                <p className="text-2xl font-black text-gray-900 dark:text-white mt-1">{totalMonitorias}</p>
              </div>
            ...
            <div className="bg-white/90 dark:bg-[#192238] border border-gray-200 dark:border-gray-800 p-4 rounded-xl flex items-center justify-between">
              <div>
                <p className="text-xs text-gray-500 dark:text-gray-400 font-semibold uppercase">Total de Inconsistências</p>
                <p className="text-2xl font-black text-red-600 dark:text-red-400 mt-1">{totalErros}</p>
              </div>
            ...
            <div className="bg-white/90 dark:bg-[#192238] border border-gray-200 dark:border-gray-800 p-4 rounded-xl flex items-center justify-between">
              <div>
                <p className="text-xs text-gray-500 dark:text-gray-400 font-semibold uppercase">Qualidade Atual</p>
                <p className={`text-2xl font-black mt-1 ${qualidadeNum >= 95 ? 'text-brand-blue-light dark:text-blue-400' : 'text-red-600 dark:text-red-400'}`}>
                  {qualidadeStr}%
                </p>
                <p className="text-[10px] text-gray-400 dark:text-gray-500">Meta: 95.0%</p>
              </div>
              <CheckCircle2 size={28} className={qualidadeNum >= 95 ? 'text-brand-blue/40 dark:text-blue-400/40' : 'text-red-500/40'} />
            </div>
          </div>

          {/* Error Tags Breakdown */}
          {sortedTags.length > 0 && (
            <div className="bg-white/70 dark:bg-[#192238] border border-gray-200 dark:border-gray-800 p-4 rounded-xl space-y-3">
              <h3 className="text-xs font-bold text-gray-700 dark:text-gray-200 uppercase tracking-wider flex items-center gap-2">
                <Tag size={14} className="text-brand-blue-light dark:text-blue-400" />
                Ofensas por Tag
              </h3>
              <div className="flex flex-wrap gap-2">
                {sortedTags.map(([tag, count]) => (
                  <div key={tag} className="bg-white dark:bg-[#131b2e] border border-gray-200 dark:border-gray-700 px-3 py-1.5 rounded-lg flex items-center gap-2 text-xs">
                    <span className="text-gray-800 dark:text-gray-200 font-medium">{tag}:</span>
                    <span className="font-bold text-red-600 dark:text-red-400 bg-red-500/10 px-2 py-0.5 rounded border border-red-500/20">{count} erro(s)</span>
                  </div>
                ))}
              </div>
            </div>
          ...
          <div className="space-y-3">
            <h3 className="text-xs font-bold text-gray-700 dark:text-gray-200 uppercase tracking-wider">Histórico de Erros</h3>
            <div className="bg-white dark:bg-[#131b2e] border border-gray-200 dark:border-gray-800 rounded-xl overflow-hidden">
              <table className="w-full text-left text-xs text-gray-700 dark:text-gray-300">
                <thead className="bg-white dark:bg-[#192238] text-gray-500 dark:text-gray-400 uppercase font-extrabold tracking-wider border-b border-gray-200 dark:border-gray-800">
                  <tr>
                ...
                <tbody className="divide-y divide-gray-200 dark:divide-gray-800 font-medium">
                  {effectiveErrorList.length === 0 ? (
                    <tr>
                      <td colSpan={7} className="py-8 text-center text-gray-400 dark:text-gray-500 italic">
                    ...
                    effectiveErrorList.map((item, idx) => {
                      ...
                      return (
                        <tr 
                          ...
                          className="hover:bg-blue-50/70 dark:hover:bg-[#1c2842] transition-colors cursor-pointer"
                          title="Clique para ver os detalhes deste registro"
                        >
                          <td className="py-2.5 px-3 font-mono text-[11px] text-gray-700 dark:text-gray-300">{formatDateToBR(itemData)}</td>
                          <td className="py-2.5 px-3 text-gray-800 dark:text-gray-200">{itemEsteira}</td>
                          <td className="py-2.5 px-3">
                            <span className="px-2 py-0.5 rounded bg-gray-100 dark:bg-gray-800 text-gray-800 dark:text-gray-200 text-[11px]">
                              {itemTag}
                            </span>
                          </td>
                          <td className="py-2.5 px-3 text-gray-500 dark:text-gray-400 text-[11px]">{itemMacro}</td>
                          <td className="py-2.5 px-3 text-center whitespace-nowrap">
                            <span className="inline-flex items-center justify-center px-2 py-0.5 rounded-full text-[10px] font-extrabold whitespace-nowrap bg-red-50 dark:bg-red-950/60 text-red-600 dark:text-red-400 border border-red-200 dark:border-red-800">
                              0% • Erro
                            </span>
                          </td>
                          <td className="py-2.5 px-3 text-center text-gray-500 dark:text-gray-400 font-mono text-[11px]">{formatDateToBR(itemFeedback)}</td>
                          <td className="py-2.5 px-3 text-gray-700 dark:text-gray-300 max-w-xs truncate" title={itemPlano}>{itemPlano}</td>
                        </tr>
                      );
                    })
                  )}
