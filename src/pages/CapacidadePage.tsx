    ...
    <div className="p-3 sm:p-4 space-y-4 bg-gray-50 dark:bg-[#0b0f19] text-gray-900 dark:text-white w-full max-w-full text-xs animate-in fade-in duration-200">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
        ...
        <div className="bg-white dark:bg-[#131b2e] border border-gray-200 dark:border-gray-800 p-3 rounded-lg shadow-2xs hover:border-[#001E62]/40 dark:hover:border-blue-500/40 transition-all flex flex-col justify-between">
          <p className="text-[10px] text-gray-500 dark:text-white font-bold uppercase tracking-wider">VOLUME PRODUZIDO</p>
          <div className="mt-1 flex items-baseline justify-between">
            ...
            <div className={`flex items-center gap-1 px-1.5 py-0.5 rounded-md font-bold text-[9px] border uppercase ${
              ...
              {parseFloat(latestMonth.momGrowth) >= 0 ? <ArrowUpRight size={10} /> : <ArrowDownRight size={10} />}
        ...
        <div className="bg-white dark:bg-[#131b2e] border border-gray-200 dark:border-gray-800 p-3 rounded-lg shadow-2xs hover:border-[#001E62]/40 dark:hover:border-blue-500/40 transition-all flex flex-col justify-between">
          <p className="text-[10px] text-gray-500 dark:text-white font-bold uppercase tracking-wider">PREVISÃO DE FECHAMENTO</p>
          <div className="mt-1 flex items-baseline justify-between">
            <h3 className="text-2xl font-black text-[#001E62] dark:text-blue-400 tracking-tight">{provisaoFechamento.toLocaleString('pt-BR')}</h3>
            <span className="text-[9px] font-bold text-slate-500 bg-slate-100 dark:bg-[#192238] px-1.5 py-0.5 rounded-md uppercase">{latestMonth.avgDailyRate.toLocaleString('pt-BR')} / dia útil</span>
          </div>
        </div>

        {/* KPI 3: Capacidade Nominal */}
        <div className="bg-white dark:bg-[#131b2e] border border-gray-200 dark:border-gray-800 p-3 rounded-lg shadow-2xs hover:border-[#001E62]/40 dark:hover:border-blue-500/40 transition-all flex flex-col justify-between">
          <p className="text-[10px] text-gray-500 dark:text-white font-bold uppercase tracking-wider">CAPACIDADE NOMINAL</p>
          <div className="mt-1 flex items-baseline justify-between">
            <h3 className="text-2xl font-black text-brand-blue dark:text-blue-400 tracking-tight">{capacidadeNominal.toLocaleString('pt-BR')}</h3>
            <span className="text-[9px] font-bold text-brand-blue bg-blue-50 dark:bg-blue-950/40 px-1.5 py-0.5 rounded-md uppercase">{capacityTableData.totals.contratados} Contratados</span>
          </div>
        </div>

        {/* KPI 4: Balanço Diário */}
        <div className="bg-white dark:bg-[#131b2e] border border-gray-200 dark:border-gray-800 p-3 rounded-lg shadow-2xs hover:border-[#001E62]/40 dark:hover:border-blue-500/40 transition-all flex flex-col justify-between">
          <p className="text-[10px] text-gray-500 dark:text-white font-bold uppercase tracking-wider">BALANÇO DIÁRIO</p>
          {(() => {
            ...
            return (
              <div className="mt-1 flex items-baseline justify-between">
                ...
                <span className={`text-[9px] font-bold px-1.5 py-0.5 rounded-md uppercase border ${
                  isPositive ? 'text-emerald-700 dark:text-emerald-300 bg-emerald-50 dark:bg-emerald-950/40 border-emerald-300' : 'text-red-700 dark:text-red-300 bg-red-50 dark:bg-red-950/40 border-red-300'
                }`}>
                  {isPositive ? 'Atingida' : 'Abaixo'}
                </span>
              </div>
            );
          })()}
      ...
      <div className="bg-white dark:bg-[#131b2e] border border-gray-200 dark:border-gray-800 p-4 rounded-lg shadow-2xs space-y-3">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          <div>
            ...
            <p className="text-[10px] text-gray-400">Comparativo de volumetria total por mês, e projeção de fechamento para o mês atual</p>
          </div>

          {/* Fixed Legend opposite Title */}
          <div className="flex items-center gap-4 bg-gray-50/70 dark:bg-[#192238] border border-gray-200 dark:border-gray-700 px-2.5 py-1 rounded-md text-[10px] font-bold uppercase self-start sm:self-auto flex-shrink-0">
            <div className="flex items-center gap-1.5 text-brand-blue dark:text-gray-200">
              <span className="w-2.5 h-2.5 rounded-sm bg-white dark:bg-white inline-block border border-gray-300 dark:border-gray-600" />
        ...
        <div className="h-72">
          <ResponsiveContainer width="100%" height="100%">
            <ComposedChart data={momData} margin={{ top: 25, right: 30, left: 15, bottom: 5 }}>
              ...
              <XAxis dataKey="label" stroke="#94a3b8" tick={{ fontSize: 10, fill: '#cbd5e1' }} padding={{ left: 25, right: 25 }} />
              ...
              <Bar dataKey="volumeRealizado" name="Volume Realizado" fill="#ffffff" radius={[4, 4, 0, 0]}>
                <LabelList dataKey="volumeRealizado" position="top" offset={6} fill="#ffffff" fontSize={10} fontWeight="bold" />
              </Bar>
              <Line type="monotone" dataKey="provisaoProjetada" name="Previsão (Projeção Fechamento)" stroke="#38bdf8" strokeWidth={3} strokeDasharray="4 4" dot={{ fill: '#38bdf8', r: 5 }}>
                <LabelList dataKey="provisaoProjetada" position="top" offset={14} fill="#38bdf8" fontSize={10} fontWeight="bold" />
              </Line>
            </ComposedChart>
          </ResponsiveContainer>
        </div>
